...
public partial class ProjectDetailPageModel(
	...
	: ObservableObject, IProjectTaskPageModel, IEnteringAware<int>
{
	...
	private async Task AddTask()
	{
		...
		await _navigationService.GoToAsync(Navigation.Relative().Push<TaskDetailPageModel>().WithIntent(new TaskDetailIntent(this._project, 0)));
	}
	...
	private Task NavigateToTask(ProjectTask task) =>
		_navigationService.GoToAsync(Navigation.Relative().Push<TaskDetailPageModel>().WithIntent(new TaskDetailIntent(this._project, task.ID)));
	...
	public async ValueTask OnEnteringAsync(int id)
    {
        if (id > 0)
		{
